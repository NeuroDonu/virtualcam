...
#define MS_PER_S 1000
    ...
    // Try external frame pump (Rust FFI) first. If a fresh frame is available,
    // copy it into the MF sample buffer and return — skipping D2D/WIC generation.
    ...
    if (SUCCEEDED(pumpHr) && pumpHr != S_FALSE) {
        WINTRACE(L"FramePump: copied external frame seq");
        buffer2D->Unlock2D();
        _frame++;
        sample->AddRef();
        *outSample = sample;
    ...
    // Fallback to original WIC + D2D render if pump not connected or no new frame.
