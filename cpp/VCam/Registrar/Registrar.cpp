...
int APIENTRY wWinMain(_In_ HINSTANCE hInstance, _In_opt_ HINSTANCE hPrevInstance, _In_ LPWSTR lpCmdLine, _In_ int nCmdShow)
{
	...
	const bool headless = lpCmdLine && wcsstr(lpCmdLine, L"/headless");
	...
	if (hwnd)
	{
		...
		if (SUCCEEDED(MFStartup(MF_VERSION)))
		{
			...
			if (SUCCEEDED(hr))
			{
				if (headless)
				{
					// Production mode: keep the session-lifetime camera registered
					// without displaying a dialog. The owner stops us by terminating
					// the process; Windows then tears down the session camera.
					Sleep(INFINITE);
				}
				else
				{
			...
			{
				config.pszMainInstruction = L"VCam could not start. Register VCamSource.dll first.";
				wchar_t text[1024];
